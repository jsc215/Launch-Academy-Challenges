...
puts "1. Username and Description:"
...
puts "2. Username followed by total number of followers:"
twitter_data.each do |hash|
  hash.each do |key, value|
    puts "#{hash.keys[0]}: #{value["number of followers"]}"
  end
end

puts ""

puts "3. Username and length of latest tweet:"
twitter_data.each do |hash|
  hash.each do |key, value|
    puts "#{hash.keys[0]}'s last tweet was #{value["latest tweet"].length} characters long."
  end
end

puts ""

puts "4. Username followed by total character count in last twenty tweets:"
twitter_data.each do |hash|
  hash.each do |key, value|
    char_sum = 0
    value["last twenty tweets"].each do |tweet|
      char_sum += tweet.length
    end
     puts "#{hash.keys[0]} used #{char_sum} characters in their last twenty tweets."
 end
end
...
followers = 0
most_followed_user = nil

twitter_data.each do |hash|
  hash.each do |key, value|
    if value["number of followers"] > followers
      followers = value["number of followers"]
      most_followed_user = key
    end
  end
end
puts "#{most_followed_user} has the most followers."
...
friends = 0
most_friendly_user = nil

twitter_data.each do |hash|
  hash.each do |key, value|
    if value["number of friends"] > friends
      friends = value["number of friends"]
      most_friendly_user = key
    end
  end
end
puts "#{most_friendly_user} has the most friends."
...
tweets = 0
tweet_happy_user = nil

twitter_data.each do |hash|
  hash.each do |key, value|
    if value["number of tweets"] > tweets
      tweets = value["number of tweets"]
      tweet_happy_user = key
    end
  end
end
puts "#{tweet_happy_user} has the most tweets."

puts ""

# Which users do not have a description listed in twitter_data?
puts "Which users do not have a description listed in twitter_data?"

twitter_data.each do |hash|
  hash.each do |key, value|

    if value["description"].nil?
      puts "#{key} does not have a description"
    end
  end
end
...
# "Which users have a location listed in twitter_data and which ones do not?"
puts "Which users have a location listed in twitter_data and which ones do not?"
has_location = []
no_location = []
...
twitter_data.each do |hash|
  hash.each do |key, value|
    ...
    if !value["location"].nil?
      has_location.push(key)
    else
      no_location.push(key)
    end
  end
end
